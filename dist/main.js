...
const feedbacks_js_1 = __importDefault(require("./feedbacks.js"));
...
class ModuleInstance extends base_1.InstanceBase {
    ...
    use_slip = false;
    readingWheels = false;
    howManyGroupLabels = 0;
    howManyMacroLabels = 0;
    startMacro = 0;
    wpc = 0;
    wheelsPerCategory = 0;
    wheels = [];
    ...
    updateActions() {
        (0, actions_js_1.default)(this);
    }
    updateFeedbacks() {
        (0, feedbacks_js_1.default)(this);
    }
    updateVariableDefinitions() {
        (0, variables_js_1.UpdateVariableDefinitions)(this);
    }
    updatePresets() {
        (0, presets_js_1.default)(this);
    }
    /**
    ...
    setConnectionState(isConnected) {
        ...
        this.updateStatus(isConnected ? base_1.InstanceStatus.Ok : base_1.InstanceStatus.Disconnected);
    ...
    getOsc10Socket(address, port) {
        let oscTcp = new osc_1.default.TCPSocketPort({
            address: address,
            port: port,
    ...
    setOscSocketListeners() {
        ...
        this.oscSocket.on('message', (message, self) => {
            ...
            else if ((matches = message.address.match(softkey)) &&
    ...
    doCategoryWheels(self) {
        let variableDefinitions = (0, variables_js_1.GetVariableDefinitions)(self);
        let updateDefs = {};
        let catWheels = [];
     ...
     */
    emptyEncVariables() {
        let variableDefinitions = (0, variables_js_1.GetVariableDefinitions)(this);
        let updateDefs = {};
    ...
    parseCueName(type, cueName) {
        ...
        const cuematch = /^(?<CUE_NUMBER>[\d\.]+\/[\d\.]+|[\d\.]+)?(?<CUE_LIST>\/[\d\.]+)?( (?<LABEL>.*?))? (?<DURATION>[\d\.]+)( (?<INTENSITY>[\d\.]+%))?$/;
    ...
    getDistinctParamForWheelLabel(wheel_label) {
        ...
        if (wheel_label != null && wheel_label != '') {
            ...
            if (lc_wheel_label in param_map_js_1.ParamMap) {
                distinctparam = param_map_js_1.ParamMap[lc_wheel_label];
            }
        }
        return distinctparam;
    }
}
exports.default = (0, base_1.runEntrypoint)(ModuleInstance, upgrades_js_1.default);
